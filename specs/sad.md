...
- **CI merge gates**: every change is gated by GitHub Actions running rustfmt, clippy `-D warnings`, the test suite on Windows/macOS/Linux, the `wasm32-unknown-unknown` build of `ron-core` (ADR-0002 invariant), and cargo-audit + cargo-deny (also on a schedule); these are the project-wide required status checks. Release/publishing is a separate later pipeline. Stable toolchain only; nightly `cargo fuzz` is an optional out-of-band check.
