...
//! validator emits **byte** `TextRange`s (consistent with `ron-core`'s structural
...
use ron_core::{parse, DiagnosticCode};
use ron_validate::validate_against;
...
fn one(model: &Value, type_name: &str, src: &str, code: DiagnosticCode) -> ron_core::Diagnostic {
...
fn assert_precise(src: &str, d: &ron_core::Diagnostic, byte: (usize, usize)) {
    assert_eq!(
