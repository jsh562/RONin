...
//!   `ron-validate`'s normal dependency closure. Their absence is the offline
...
//!   to be made.
//! * `jsonschema` and `serde_json` are **ABSENT** from `ron-core`'s normal
//!   dependency closure (HINT-002/AD-007: `ron-core` stays rowan-only).
//! * `ron-types` is absent from both `ron-core` and `ron-validate` (the native
...
//!   `<ws>/src/ron-validate`), so `cargo metadata` runs in the workspace.
...
/// Workspace root: this crate's manifest dir is `<ws>/src/ron-validate`.
...
#[test]
fn ron_validate_has_no_network_transport_in_normal_closure() {
    ...
    // ron-validate's normal closure — the offline proof.
    assert_absent(&graph, "ron-validate", &["reqwest", "rustls"]);

    // ron-validate MUST still actually depend on jsonschema (the engine) — a
    // sanity check that the closure walk is meaningful (not trivially empty).
    let rv_closure = graph.normal_closure_names("ron-validate");
    assert!(
        rv_closure.contains("jsonschema"),
        "expected `jsonschema` in ron-validate's normal closure (closure: {rv_closure:?})"
    );
    assert!(
        rv_closure.contains("ron-core"),
        "expected `ron-core` in ron-validate's normal closure (closure: {rv_closure:?})"
    );
}

#[test]
fn ron_core_has_no_validation_deps_in_normal_closure() {
    let Some(meta) = cargo_metadata() else {
        return; // documented skip.
    };
    let graph = Graph::from_metadata(&meta);

    // HINT-002/AD-007: ron-core stays rowan-only — jsonschema/serde_json MUST be
    // absent from its normal closure.
    assert_absent(&graph, "ron-core", &["jsonschema", "serde_json"]);

    // Sanity: ron-core's closure is non-empty (it depends on rowan) so the walk
    // is meaningful.
    let core_closure = graph.normal_closure_names("ron-core");
    assert!(
        !core_closure.is_empty(),
        "ron-core normal closure unexpectedly empty — closure walk is not meaningful"
    );
}

#[test]
fn ron_types_absent_from_wasm_clean_crates() {
    let Some(meta) = cargo_metadata() else {
        return; // documented skip.
    };
    let graph = Graph::from_metadata(&meta);
    ...
    assert_absent(&graph, "ron-core", &["ron-types"]);
    assert_absent(&graph, "ron-validate", &["ron-types"]);
}
