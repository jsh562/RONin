...
use ron_core::{parse, Diagnostic, DiagnosticCode, Severity, TextRange};
use ron_validate::{dedup_against_structural, validate, validate_against};
...
fn skip_unparseable_zero_cascade_in_span_diag_on_remainder_structural_present() {
    ...
    //   * `id: @bad` is malformed -> a `ron-core` parse-error node span.
    ...
    assert!(
        !structural.is_empty(),
        "expected `ron-core` structural diagnostics for the malformed `@bad` region"
    );
