//! `ron-validate` — WASM-clean type-aware validation for RON documents (E006).
//!
//! This crate runs a [`jsonschema`] validator over a CST→JSON projection of a
//! RON document and surfaces the results as `ron-core` diagnostics with precise
...
//!   `ron-core` (rowan-only CST), `jsonschema`, and `serde_json` only.
...
/// * `doc` — the parsed `ron-core` [`CstDocument`](ron_core::CstDocument) the
///   diagnostics' precise ranges are taken from. Read-only (FR-020).
/// * `structural` — the document's `ron-core` structural diagnostic set, used to
...
/// # Returns
///
/// The set of type [`Diagnostic`](ron_core::Diagnostic)s (each carrying a
...
pub fn validate(
    ...
    structural: &[ron_core::Diagnostic],
) -> Vec<ron_core::Diagnostic> {
