...
//! precise CST byte [`ron_core::TextRange`] — the **field-key span** for
...
fn project_literal(lit: &ron_core::syntax::ast::Literal) -> serde_json::Value {
...
fn project_enum_variant(
    v: &ron_core::syntax::ast::EnumVariant,
    builder: &mut PointerBuilder,
    index: &mut PointerRangeIndex,
) -> serde_json::Value {
...
/// than a direct token.
fn entry_key_name_and_span(entry: &ron_core::syntax::ast::MapEntry) -> Option<(String, TextRange)> {
    let key = entry.key()?;
