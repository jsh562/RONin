...
//! `RON-V` [`ronin_core::Diagnostic`]s with precise CST ranges (E006/FR-001,
...
//! one stable `RON-V####` [`ronin_core::DiagnosticCode`].
//!
//! # Encoding contract
//!
...
use ronin_core::{Diagnostic, DiagnosticCode, SyntaxKind, SyntaxNode, TextRange};
...
#[must_use]
pub fn validate_root(model: &Value, doc: &ronin_core::CstDocument) -> Vec<Diagnostic> {
...
///
/// `subtree` is a value-position [`ronin_core::SyntaxNode`] (e.g. the value node of
...
fn validate_node(target: &Value, defs: &Value, doc: &ronin_core::CstDocument) -> Vec<Diagnostic> {
    let resolved = resolve_ref(target, defs);
    ...
    // finding whose span intersects a `ronin-core` parse-error node span, so the
...
/// Collect the byte spans of every `ronin-core` parse-error node (`SyntaxKind::Error`)
...
fn error_node_spans(doc: &ronin_core::CstDocument) -> Vec<TextRange> {
